...
if(isset($_POST['update_order_status'])){
	...
	if($update_order_status=='5'){
		mysqli_query($con,"update `order` set order_status='$update_order_status',payment_status='Success'
		 where id='$order_id'");
	}else{
				   ...
				   <div class="table-stats order-table ov-h">
					  <table class="table">
						...
						<div id="address_details">
							...
							$oid=mysqli_query($con,"select `id` from `order` WHERE `id` = $order_id");
							//print_r($oid);
							...
							$result1 = mysqli_query($con,"select * from `order` WHERE `id` = $uid");
							//print_r($result1);
							...
							$result2 = mysqli_query($con,"select `name` from `user_info` WHERE `id` = $r[user_id]");
							//print_r($result2);
							...
							from order_status,`order` where `order`.id='$order_id' and `order`.order_status=
							order_status.id"));
